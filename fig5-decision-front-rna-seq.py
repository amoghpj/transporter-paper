...
for n,c, x, y in zip(nut,colors,PC[:,1],PC[:,0]):
    axphenotwin.plot(x,y,marker=plot_styles[n]['style'],markersize=plot_styles[n]['size'],c=c)
...
expdf['meandiff'] = expdf[['2-glc-cd103','2-fru-cd103','02-glc-cd103']].mean(axis=1) -\
    expdf[["002-glc-cd103", "2-mal-cd103","2-raf-cd103","2-ace-cd103"]].mean(axis=1)
...
#### Transcriptional PCA
rnaseqdf = pd.read_csv('./data/fig5-hxt-expression-pca.csv',index_col=0)
rnaseqdf = rnaseqdf[~rnaseqdf.nutrient.str.contains("gal")]
...
for n,c, x, y in zip(rnaseqdf.nutname, colors, rnaseqdf.PC1, rnaseqdf.PC2):
...
for c, x, y in zip(rnaseqdf.nutname,rnaseqdf.PC1,rnaseqdf.PC2):
    axrna.annotate("".join(c.split("-")[0:2]).lower(),xy=(x,y),fontsize=16,rotation=45)
axrna.set_title("HXT expression")
axrna.set_ylabel("PC2")
axrna.set_xlabel("PC1")
# axrna.set_xlim(-5,9)
# axrna.set_ylim(-5,7)
#### PC1 comparison
ordered1 = []
...
pccompdf = pd.DataFrame({'pcpheno':PC[:,0],'pcrna':ordered1,'strains':[c.split("-")[1] for c in collectdf.columns]})

annotcoords={'sh07':(-2,2),'cd103':(0,-2),'a28y':(6,0)}
for s in strains:
...
for x1, y1,x2 in zip(PC[:,1],PC[:,0],ordered1):
    xyA = (x2,y1)
    xyB = (x1,y1)
    con = ConnectionPatch(xyA=xyA, xyB=xyB, coordsA="data", coordsB="data",
                      axesA=axcomp, axesB=axphenotwin, color="k",alpha=0.2)
    con.set_in_layout(False)
    axcomp.add_artist(con)
for x1, y1,x2 in zip(ordered1, ordered2, PC[:,0]):
    xyB = (x1,y1)
    xyA = (x1,x2)
    con = ConnectionPatch(xyA=xyA, xyB=xyB, coordsA="data", coordsB="data",
